...
async function ensureOwnerIdentity(projectId) {
  const email = `studio-autosave-${projectId}-${randomUUID()}@example.test`.toLowerCase();
  const password = `ThemAutosave-${randomUUID()}-aA1!`;
  const signup = await postBackendAuthJSON("/auth/signup", {
    email,
    password,
    display_name: "Studio Autosave Restore Smoke",
  });
  assert(
    signup.response.ok,
    `Failed to create Studio autosave owner: ${signup.response.status} ${JSON.stringify(signup.payload)}`
  );
  ...
  assert(accessToken, "Studio autosave owner signup did not return an access token");
  assert(userId, "Studio autosave owner signup did not return a user id");
  ...
  assert(
    session.response.ok,
    `Failed to create Studio autosave client session: ${session.response.status} ${JSON.stringify(session.payload)}`
  );
  const clientToken = String(session.payload?.client_token || session.payload?.session_id || "").trim();
  assert(clientToken, "Studio autosave client session did not return a client token");
