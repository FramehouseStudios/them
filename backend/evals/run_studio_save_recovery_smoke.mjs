...
async function ensureOwnerIdentity(projectId) {
  const email = `studio-save-recovery-${projectId}-${randomUUID()}@example.test`.toLowerCase();
  const password = `ThemSaveRecovery-${randomUUID()}-aA1!`;
  const signup = await postBackendAuthJSON("/auth/signup", {
    email,
    password,
    display_name: "Studio Save Recovery Smoke",
  });
  assert(
    signup.response.ok,
    `Failed to create Studio save-recovery owner: ${signup.response.status} ${JSON.stringify(signup.payload)}`
  );
  ...
  assert(accessToken, "Studio save-recovery owner signup did not return an access token");
  assert(userId, "Studio save-recovery owner signup did not return a user id");
  ...
  assert(
    session.response.ok,
    `Failed to create Studio save-recovery client session: ${session.response.status} ${JSON.stringify(session.payload)}`
  );
  const clientToken = String(session.payload?.client_token || session.payload?.session_id || "").trim();
  assert(clientToken, "Studio save-recovery client session did not return a client token");
