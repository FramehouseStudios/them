...
test("[feature-screenplay-map] carries feature spine, promises, and Act III payoff path", () => {
  ...
  assert.ok(block.includes("memory_to_page_execution:"));
  assert.ok(block.includes("purpose: convert persistent feature memory into immediate playable screenplay behavior"));
  assert.ok(block.includes("inherited_state: The public betrayal destroyed her safe legal tactic."));
  assert.ok(block.includes("act_pressure_to_dramatize: All-is-lost pressure must make the false victory morally unusable."));
  assert.ok(block.includes("character_arc_pressure: Mara is learning that control is not the same as loyalty."));
  assert.ok(block.includes("turn_runway_to_dramatize:"));
  assert.ok(block.includes("turn_1: Mara loses the public case."));
  assert.ok(block.includes("turn_2: The sister's voicemail reframes the cover-up."));
  assert.ok(block.includes("payoff_path_to_aim_toward:"));
  assert.ok(block.includes("story_threads_to_touch:"));
  assert.ok(block.includes("arc_turns_to_make_visible:"));
  assert.ok(block.includes("image_motifs_to_use_as_action:"));
  assert.ok(block.includes("Dramatize turn_1 before inventing a new plot lane"));
  assert.ok(block.includes("Never print these memory labels in the answer"));
...
test("[feature-screenplay-map] emits page-batch execution plan for feature page requests", () => {
  ...
  assert.ok(block.includes("page_target_sizing:"));
  assert.ok(block.includes("purpose: turn page count into a dramaturgical runway"));
  assert.ok(block.includes("target_window: p48-p57 / 110"));
  assert.ok(block.includes("pages_remaining_after_batch: 53"));
  assert.ok(block.includes("start_sequence: Act II - Midpoint Pressure"));
  assert.ok(block.includes("end_sequence: Act II - Reversal Fallout"));
  assert.ok(block.includes("sequence_boundary_rule: if the batch crosses into Act II - Reversal Fallout"));
  assert.ok(block.includes("scene_turn_budget: 4 escalating turns"));
  assert.ok(block.includes("story_state_change_floor: at least 5 visible leverage/reveal/cost/tactic shifts"));
  assert.ok(block.includes("sizing_rule: if model space is tight"));
  assert.ok(block.includes("continuation_quality_floor:"));
  assert.ok(block.includes("Open from inherited emotional residue as visible behavior"));
  assert.ok(block.includes("Preserve supplied next_three_turns and next_scene_plan before adding new plot."));
