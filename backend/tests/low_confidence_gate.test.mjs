...
test("[low-confidence-gate] echo fragments look unintelligible, short commands do not", () => {
  for (const junk of ["", "iz", "ik", "I...", "Ai...", "卡。", "ครับ", "toate."]) {
    assert.equal(looksLikeUnintelligibleFragment(junk), true, JSON.stringify(junk));
  }
  for (const real of ["Talk to me.", "Next beat", "Rewrite it", "You are", "Write a scene", "Do you hear me?"]) {
...
test("[low-confidence-gate] heuristic-only confidence never bounces a real short command", () => {
  ...
  assert.equal(shouldTreatAsLowConfidence({ ambiguous: true, sttJson: heuristicOnly, transcript: "ครับ" }), true);
  assert.equal(shouldTreatAsLowConfidence({ ambiguous: true, sttJson: heuristicOnly, transcript: "iz" }), true);
