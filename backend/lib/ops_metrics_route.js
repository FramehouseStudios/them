  ...
  app.get("/ops/metrics", (req, res) => {
    ...
    const recent = samples
      ...
      .map((sample) => ({
        ...
        lane: sample.lane,
        model: sample.model,
        model_tier: sample.modelTier || "unknown",
        model_reason: sample.modelReason || "unknown",
        reasoning_effort: sample.reasoningEffort || "none",
        model_fallback: sample.modelFallback ? 1 : 0,
        input_tokens: Math.max(0, Number(sample.inputTokens || 0)),
        output_tokens: Math.max(0, Number(sample.outputTokens || 0)),
        reasoning_tokens: Math.max(0, Number(sample.reasoningTokens || 0)),
        structural_quality_applicable: sample.structuralQualityApplicable ? 1 : 0,
        structural_quality_passed: sample.structuralQualityPassed ? 1 : 0,
        structural_quality_repaired: sample.structuralQualityRepaired ? 1 : 0,
        structural_quality_outcome: sample.structuralQualityOutcome || "not_applicable",
        structural_quality_reason: sample.structuralQualityReason || "none",
        structural_quality_initial_score: Math.max(0, Number(sample.structuralQualityInitialScore || 0)),
        structural_quality_final_score: Math.max(0, Number(sample.structuralQualityFinalScore || 0)),
        structural_repair_ms: Math.max(0, Number(sample.structuralRepairMs || 0)),
