...
| --- | --- | --- | --- | --- |
| [#134](https://github.com/FramehouseStudios/them/pull/134) | T-ops-health-summary-route | 1 | blocked | Has `do-not-merge`; still conflicts with post-T62 `main` in `backend/index.js`. Claude should rebase/update, preserve the safe-public/no-user-data posture, rerun focused route test plus `npm test`, then request review. |
| [#135](https://github.com/FramehouseStudios/them/pull/135) | T-screenplay-export-formats-list-route | 1 | review | New `GET /screenplay/export/formats` returns the canonical frozen format set (fountain/txt/fdx/md/markdown supported + pdf unsupported), each with extension/mediaType/description. Cache-Control: no-store. iOS can stop hard-coding the set. 7 unit tests; npm test green. |
| [#141](https://github.com/FramehouseStudios/them/pull/141) | T-prompt-assembly-block-signal-cap-eval | 1 | review | New `npm run eval:block-signal-block-cap` — bounds the `<block_signal>` block (and the assembled prompt with one) under pathological summary input. Catches future unbounded-summary regressions. No production code touched. |
| [#142](https://github.com/FramehouseStudios/them/pull/142) | T-known-domains-startup-check | 1 | review | New `lib/known_domains_startup_check.js`, wired from `index.js`'s `SHOULD_START_SERVER` branch. Runs PR #115's invariants at boot — logs by default, throws when called with `throwOnError=true`. Catches deploy-time corruption before the first persistence call. |
| [#143](https://github.com/FramehouseStudios/them/pull/143) | T-decisions-queue-fixture-template | 1 | review | New `docs/decisions-queue-template.md` — canonical copy-paste skeleton for queue entries matching PR #104's parser + PR #127's lint. Includes required/optional field rules, a worked example, and anti-examples. Docs-only. |
| [#144](https://github.com/FramehouseStudios/them/pull/144) | T-coordination-state-cli-validate | 1 | review | New `validate` subcommand on `scripts/coordination_state.mjs` — same invariants as PR #117's standalone schema check, surfaced next to read/open-prs/blockers/decisions. Smoke test verifies exit codes. |
| [#133](https://github.com/FramehouseStudios/them/pull/133) | T-build-tasks-md-anchors | 1 | blocked | Has `do-not-merge`; after merging post-T62 `main`, the generator still rewrites `TASKS.md`. Claude should commit the current generated `TASKS.md`, verify a second `node scripts/build_tasks_md.mjs --write` is clean, then request review. |
...
- Claude PRs #74, #76, #79-#88, #90, #92, #97, #99, #100, #104, #105, #107, #110-#112, #115, #117, #124, #127, #133, and #134 currently carry `do-not-merge` or `needs-human`; Claude should clear those before opening more backend feature branches.
