...
| [#159](https://github.com/FramehouseStudios/them/pull/159) | T-ops-health-summary-eval | 1 | review | New `npm run eval:ops-health-summary` — mounts /ops/health-summary with the exact features map from index.js and pins the canonical 6-key set. 16 checks; npm test green. |
| [#160](https://github.com/FramehouseStudios/them/pull/160) | T-archetype-engine-canon-eval | 1 | review | New `npm run eval:archetype-canon` — pins the 8 canonical archetype labels (hero/mentor/shadow/trickster/ally/herald/threshold_guardian/shapeshifter), asserts `ARCHETYPES` frozen, required per-entry fields. Catches silent renames that would regress iOS T48. |
| [#161](https://github.com/FramehouseStudios/them/pull/161) | T-trait-library-canon-eval | 1 | review | New `npm run eval:trait-library-canon` — pins the 23-keyword `TRAIT_KEYWORDS` set + structural caps. iOS surfaces these labels directly; silent change would re-classify the user's character roster. |
| [#163](https://github.com/FramehouseStudios/them/pull/163) | T-twist-engine-canon-eval | 1 | review | New `npm run eval:twist-engine-canon` — probes every (framework, beat) pair via `suggestTwists()` and asserts 4 frameworks, beat sets match main, **42 unique twist IDs**, severity enum, unknown framework throws. iOS T37 tracks pinned/dismissed by these IDs. |
| [#164](https://github.com/FramehouseStudios/them/pull/164) | T-block-detector-canon-eval | 1 | review | New `npm run eval:block-detector-canon` — pins the block-signal envelope, `SIGNAL_WEIGHTS` 4-key frozen set summing to 1.0, and the level thresholds. iOS T35 + T53 both read this envelope. |
