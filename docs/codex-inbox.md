...
## Recently cleared (round 14 — 2026-05-12)
...
| --- | --- | --- |
| [#76](https://github.com/FramehouseStudios/them/pull/76) | T-logline-drift-alert | Rebased; all 25 logline-distiller tests green. |
...
| [#97](https://github.com/FramehouseStudios/them/pull/97) | T-talk-turn-meta-stats | Added safe-public access-control posture: no-leakage + canonical-keys tests. |
| [#100](https://github.com/FramehouseStudios/them/pull/100) | T-talk-error-rate-tracker | Reworked storage to per-event timestamp ring; `sinceMs` now counts only window events (not class lifetime). Added safe-public posture + 3 no-leakage tests. 16/16 tests green. |
...
The earlier `ops-surface-access-control` cross-PR blocker is fully cleared:
both `/talk/stats` (#97) and `/talk/errors` (#100) now carry an explicit
safe-public posture documented in both the lib module header and the mount
site, with no-leakage tests pinning the response shape.
