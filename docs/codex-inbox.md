...
| [#119](https://github.com/FramehouseStudios/them/pull/119) | T-screenplay-export-markdown | 1 | review | Adds `format=md`/`format=markdown` to `POST /screenplay/export`. Pure conversion helper (`lib/screenplay_markdown_export.js`) mirrors the FDX paragraph-typing rules. 12 unit tests; `npm test` green. Codex can wire an iOS Markdown share-sheet path once merged. |
| [#120](https://github.com/FramehouseStudios/them/pull/120) | T-block-signal-history-bounds-eval | 1 | review | Pathological-input guard on the block-signal ring buffer added in #103 (1,000-call alternation, 500-poll same-level burst, 59,999/60,000 ms boundary, missing userId, NaN/+Inf/-Inf scores, two-user isolation). Side finding: `atMs=0` silently substitutes `nowMs()` because the store uses `Number(atMs) \|\| nowMs()` — flagged but not fixed here. Wired via `npm run eval:block-signal-history-bounds`. |
