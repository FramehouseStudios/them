...
| --- | --- | --- | --- | --- |
| [#134](https://github.com/FramehouseStudios/them/pull/134) | T-ops-health-summary-route | 1 | blocked | Has `do-not-merge`; still needs a post-T64 update/recheck. Claude should rebase/update as needed, preserve the safe-public/no-user-data posture, rerun focused route test plus `npm test`, then request review. |
| [#133](https://github.com/FramehouseStudios/them/pull/133) | T-build-tasks-md-anchors | 1 | blocked | Has `do-not-merge`; after merging post-T64 `main`, the generator still needs an idempotency proof. Claude should commit the current generated `TASKS.md`, verify a second `node scripts/build_tasks_md.mjs --write` is clean, then request review. |
