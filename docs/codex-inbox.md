...
| [#103](https://github.com/FramehouseStudios/them/pull/103) | T-block-signal-history-tracking | 1 | review | Adds `recordBlockSignalSample` to creative-memory store and wires it into `GET /memory/block-signal`. Appends one sample per non-debounced call (60s same-level debounce, 30-entry ring buffer). Pure additive field on `habits.block_signal_history`; iOS sparkline / "stuck this week?" surfaces become possible without re-architecting. |
| [#104](https://github.com/FramehouseStudios/them/pull/104) | T-decisions-queue-route | 1 | review | New `GET /coordination/decisions-queue` exposes `docs/decisions-queue.md` as JSON (`{ schemaVersion, open[], resolved[], counts }`). Markdown stays canonical; this is a read-only projection so iOS / dashboards / Codex automation can surface unanswered items without re-parsing the ledger. |
| [#105](https://github.com/FramehouseStudios/them/pull/105) | T-memory-quality-eval | 1 | review | Adds `npm run eval:memory-quality` — multi-turn deterministic eval that asserts the creative-memory recall loop closes across 5 turns (character → tone → voice trait → multi-character), plus per-user isolation and determinism. No production code touched; safe to wire into the eval gate. |
