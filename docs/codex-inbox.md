...
| --- | --- | --- | --- | --- |
| [#148](https://github.com/FramehouseStudios/them/pull/148) | T-ops-routes-list-route | 1 | merged | Curated route manifest landed (commit a691a33). iOS / API can now consume `GET /ops/routes` for the canonical app-facing route set. |
| [#133](https://github.com/FramehouseStudios/them/pull/133) | T-build-tasks-md-anchors | 1 | merged | Anchors + matcher hardening + idempotency proof landed (commit ce658d3). `node scripts/build_tasks_md.mjs --write` is now stable across consecutive runs. |
| [#142](https://github.com/FramehouseStudios/them/pull/142) | T-known-domains-startup-check | 1 | merged | Boot-time KNOWN_DOMAINS invariants landed (commit 1abfc40), hardened with `Array.isArray` guard + `domainsForTest` injection + 13 regression tests. |
...
| [#87](https://github.com/FramehouseStudios/them/pull/87) | T-screenplay-import-fountain | 1 | merged | Route-local 413 handler landed (commit 97a64c9). Middleware responds directly with structured JSON instead of letting the error escape to Express's default handler. Stream-side cap added so callers can't bypass Content-Length. |
...
| [#90](https://github.com/FramehouseStudios/them/pull/90) | T-fdx-export-endpoint | 1 | review | Route-local `express.json({ limit: "2mb" })` added; test fixture no longer installs app-level parser. Two production-style tests build a bare Express app (no middleware) and POST JSON through the live route. Ready for Codex re-review. |
