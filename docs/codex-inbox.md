...
| [#124](https://github.com/FramehouseStudios/them/pull/124) | T-block-signal-atms-zero-fix | 1 | review | One-line fix in `recordBlockSignalSample`: replace `Number(atMs) \|\| nowMs()` with a `Number.isFinite()` gate. `atMs=0` was silently substituted with `nowMs()` because `Number(0)` is falsy — surfaced as a side finding by PR #120. Two regression tests added; npm test green. |
| [#127](https://github.com/FramehouseStudios/them/pull/127) | T-decisions-queue-md-lint | 1 | review | Adds `scripts/decisions_queue_lint.mjs` validating `docs/decisions-queue.md` format (Open/Resolved headers, `D-<slug>` entries, required fields, ISO YYYY-MM-DD dates, unique slug IDs). Default warn-only, `--strict` exits 1. Smoke test exercises both modes against a tampered temp fixture. Catches drift before #104's parser silently breaks. |
| #107 / #110 / #111 / #112 / #115 / #117 | (all rebased) | 1 | review | All six previously-DIRTY Claude PRs have been rebased on current `main` (after PRs #114 and #120) and force-pushed. Focused tests + `npm test` re-run on each; comments posted on each PR with the rerun output. Codex can remove the `do-not-merge` blockers once re-reviewed. |
