...
| [#166](https://github.com/FramehouseStudios/them/pull/166) | T-format-linter-rules-canon-eval | 1 | review | Pins the canonical 8-rule set emitted by `lintScreenplay()` + envelope + severity enum + line-ascending sort + determinism. iOS T28 reads each `rule` ID as a dedupe / dismiss key. |
| [#168](https://github.com/FramehouseStudios/them/pull/168) | T-creative-memory-store-eviction-eval | 1 | review | Pathological-input guard on `recordCharacterMention`: 50 "June" mentions → 1 record (de-dup), whitespace-padded names collapse, 200 unique names → exactly `CHARACTERS_MAX = 32` stored (Char199 in, Char000 out), `last_referenced` updates, empty/whitespace/undefined → skipped. |
| [#169](https://github.com/FramehouseStudios/them/pull/169) | T-coordination-state-mutate-eval | 1 | review | Round-trip eval that runs every mutating subcommand of `coordination_state.mjs` (add-pr/set-pr/close-pr/add-blocker/clear-blocker/add-decision/clear-decision) against a temp fixture and asserts `validate` exits 0 after every mutation. |
| [#170](https://github.com/FramehouseStudios/them/pull/170) | T-talk-turn-rate-limit-route | 1 | review | Mounts the [#154](https://github.com/FramehouseStudios/them/pull/154) token-bucket limiter as an opt-in middleware on `GET /talk/turn/:turnId`. Backwards-compatible. Denied → 429 with `retry_after_ms` + `Retry-After` header; runs before any other gate. Production `index.js` mount unchanged until thresholds are decided. |
| [#171](https://github.com/FramehouseStudios/them/pull/171) | T-eval-gate-add-canon-evals | 1 | review | Umbrella `npm run eval:canon` chaining all merged canon evals. Future canon evals land as `&&` clauses as their PRs merge. |
