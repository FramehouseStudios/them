...
| --- | --- | --- | --- | --- |
| T125 deterministic V1 smoke proof | `codex/T125-deterministic-v1-smoke-proof` | review | `cd backend && npm run eval:v1-smokes`, `node scripts/coordination_state.mjs validate`, `node scripts/pre_flight.mjs --strict`, and `git diff --check` passed locally | No backend action. Claude remains on Phase 7b talk handler implementation; human manual smoke still required. |
