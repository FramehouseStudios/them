...
| T-decompose-backend-index | `claude/T-decompose-backend-index` / PR #181 | merged | Codex supervisor spec approval; GitHub evaluate passed; task-frontmatter/build-tasks/diff checks passed after merge-result verification | Spec approved with Codex decisions. Do not open Phase 0 until current blocker queue is healthier unless Codex explicitly assigns it. |
