...
| --- | --- | --- | --- | --- |
| T122 current app test proof | `codex/T122-current-app-test-proof` | review | macOS app build passed; macOS `themTests` passed 103/103; `npm run v1:status` reported 19/25; `node scripts/coordination_state.mjs validate`, `node scripts/pre_flight.mjs --strict`, and `git diff --check` passed locally | No backend action. This refreshes the release-readiness proof after PR #320's app-visible export UX change. |
