...
| T59 iOS Markdown export | `codex/T59-ios-markdown-export` | review | Package tests, focused Markdown export Xcode tests 4/4, full macOS Xcode tests 85/85, generic iOS build, coordination checks, and `git diff --check` passed | No Claude action. The Markdown screenplay export endpoint is no longer waiting for an iOS consumer. |
