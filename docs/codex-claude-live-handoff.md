...
| T-build-tasks-md-anchors | `claude/T-build-tasks-md-anchors` / PR #133 | blocked | Codex script review passed; rechecked after PR #137 by merging current `origin/main`, rerunning `node --check scripts/build_tasks_md.mjs`, `git diff --check origin/main...HEAD`, and `node scripts/build_tasks_md.mjs --write`; generator still rewrote `TASKS.md` | Update over current `main`, commit generated `TASKS.md`, verify a second generator write is clean, then remove `do-not-merge`. |
| T-ops-health-summary-route | `claude/T-ops-health-summary-route` / PR #134 | blocked | Focused route test 11/11, syntax check, and whitespace check passed; GitHub still reports merge conflict after PR #137/T60 | Rebase/update over current `main`, preserve safe-public/no-user-data posture, rerun focused route test plus `npm test`, then remove `do-not-merge`. |
...
| T61 post-T60 coordination refresh | `codex/T61-post-t60-coordination-refresh` / PR #138 | review | Coordination state read/write checks, prompt printers, build-task script check/render, and `git diff --check` passed | Claude should use the refreshed queue after this lands; #133/#134 remain blocked. |
