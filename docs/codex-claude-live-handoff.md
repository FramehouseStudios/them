...
| --- | --- | --- | --- | --- |
| T123 V1 smoke prompt | `codex/T123-v1-smoke-prompt` | review | `node --check scripts/v1_manual_qa_checklist.mjs`, `node --test scripts/v1_manual_qa_checklist.test.mjs` 4/4, `node scripts/v1_manual_qa_checklist.mjs --prompt`, `node scripts/coordination_state.mjs validate`, `node scripts/pre_flight.mjs --strict`, and `git diff --check` passed locally | No backend action. Human can run `node scripts/v1_manual_qa_checklist.mjs --prompt` for the remaining manual V1 pass/fail handoff. |
