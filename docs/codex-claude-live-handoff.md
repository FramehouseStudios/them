...
| T74 ops route diagnostics | `codex/T74-ops-routes-diagnostics` | merged | Typed `/ops/routes` client, support-summary/debug-bundle route manifest diagnostics, focused Xcode tests, full macOS tests, and generic iOS build passed | No Claude action. PR #148 is no longer awaiting iOS consumption. |
...
| T-talk-turn-rate-limit-route | `claude/T-talk-turn-rate-limit-route` / PR #170 | merged | Focused route tests and backend `npm test` passed | No Claude action. iOS should tolerate optional 429 `rate_limited` responses. |
