...
| --- | --- | --- | --- | --- |
| T143 core-only memory export decision | `codex/T143-memory-export-core-decision` / PR #345 | merged | Codex posted the supervisor decision on PR #94 and recorded it in the event lane/coordination state: V1 memory export must drop `projectIds`, `loglineHistory`, and `acceptedTwists` until ownership scoping exists. GitHub `evaluate` passed and Codex merged under D005 on 2026-05-16. | Revise PR #94 to the core-only V1 export contract, update tests/docs, add a post-V1 ownership-scoped project export follow-up, rebase on current main, and rerun focused export tests plus backend `npm test`. |
| T141 safe local release config handoff | `codex/T141-release-config-local-runbook` / PR #343 | merged | Adds ignored `them/Release.local.env` flow, checked-in template, `scripts/run_release_preflight.sh`, launch-room command update, and release docs so real Development Team, hosted `BACKEND_URL`, and `APP_TOKEN` can clear preflight without committing secrets. Verification: shell syntax checks passed; missing/placeholder local config failures are clear; expected-red appstore preflight still reports `fail=3 warn=1`; focused runner tests passed 2/2; launch-room tests passed 5/5; coordination validation, strict pre-flight, and `git diff --check` passed; GitHub `evaluate` passed and Codex merged under D005 on 2026-05-16. | Stay in V1 smoke-failure support mode. No backend action unless a concrete manual-smoke failure appears after real release config is supplied. |
| T139 V1 release smoke clearance | `codex/T139-v1-release-smoke-clearance` / PR #341 | merged | Release audit found no `DEVELOPMENT_TEAM_ID`, no release `BACKEND_URL`, no `APP_TOKEN`, and `0 valid identities found`; macOS build passed, macOS `themTests` passed 108/108, generic unsigned iOS build passed, `scripts/appstore_preflight.sh` failed with `fail=3 warn=1`, and `docs/v1-launch-doctor.latest.json` now exists as a blocked `not_started` report. GitHub `evaluate` passed and Codex merged under D005 on 2026-05-16. | Stay in V1 smoke-failure support mode. Do not open net-new backend work. If Codex or the human posts a concrete Talk/Studio/Memory/Realtime Launch Doctor failure after real release config is present, fix that exact failure first. |
