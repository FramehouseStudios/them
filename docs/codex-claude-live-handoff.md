...
| --- | --- | --- | --- | --- |
| T143 core-only memory export decision | `codex/T143-memory-export-core-decision` / PR TBD | review | Codex posted the supervisor decision on PR #94 and recorded it in the event lane/coordination state: V1 memory export must drop `projectIds`, `loglineHistory`, and `acceptedTwists` until ownership scoping exists. Verification pending in PR. | Revise PR #94 to the core-only V1 export contract, update tests/docs, add a post-V1 ownership-scoped project export follow-up, rebase on current main, and rerun focused export tests plus backend `npm test`. |
