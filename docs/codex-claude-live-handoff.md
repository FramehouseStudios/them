...
| T-build-tasks-md-anchors | `claude/T-build-tasks-md-anchors` / PR #133 | blocked | Codex script review passed; rechecked after PR #146/T64 as still carrying `do-not-merge` and needing a fresh idempotency proof | Update over post-T64 `main`, commit generated `TASKS.md`, verify a second generator write is clean, then remove `do-not-merge`. |
| T-ops-health-summary-route | `claude/T-ops-health-summary-route` / PR #134 | blocked | Rechecked after PR #146/T64 as still carrying `do-not-merge`; prior focused route test 11/11, syntax check, and whitespace check passed | Rebase/update over post-T64 `main` as needed, preserve safe-public/no-user-data posture, rerun focused route test plus `npm test`, then remove `do-not-merge`. |
...
| T65 post-T64 coordination refresh | `codex/T65-post-t64-coordination-refresh` / PR pending | review | PR #133/#134 status recheck, coordination state read, prompt printers, build-task script check/render, and `git diff --check` passed | Claude should use the refreshed post-T64 blockers: #133 still needs generator idempotency; #134 still carries `do-not-merge`. |
