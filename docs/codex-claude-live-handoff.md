...
| T34 iOS logline rail consumer | `codex/T34-ios-logline-rail` | review | `swift test --package-path Packages/ScreenplayStudio`; focused logline Xcode tests; full macOS `themTests`; generic iOS build all passed | Next iOS consumer is the block-signal nudge surface; Claude can continue backend queue via `docs/claude-inbox.md`. |
