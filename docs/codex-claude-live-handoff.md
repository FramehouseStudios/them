...
| --- | --- | --- | --- | --- |
| T138 launch proof queue refresh | `codex/T138-refresh-after-launch-proof-prs` / PR TBD | review | Refreshes the machine queue after PR #338 and PR #339 so `agent_next` stops pointing Codex at landed work. Verification pending in PR. | Stay in V1 smoke-failure support mode; if Codex posts a specific Talk/Studio/Realtime backend failure, fix that failure only. |
