...
| T-screenplay-export-markdown | `claude/T-screenplay-export-markdown` / PR #119 | merged | Codex diff review; route-level Markdown coverage added; GitHub evaluate passed; Claude reported Markdown unit/route tests and backend `npm test` 361 pass / 1 skipped | Codex should add an iOS Markdown export/share option that sends `format=md` to `POST /screenplay/export`. |
