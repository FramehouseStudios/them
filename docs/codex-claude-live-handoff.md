...
| T-ops-health-summary-route | `claude/T-ops-health-summary-route` / PR #134 | blocked | Rechecked after PR #149 as still conflicting/carrying `do-not-merge`; prior focused route test 11/11, syntax check, and whitespace check passed | Rebase/update over post-#149 `main`, preserve safe-public/no-user-data posture, rerun focused route test plus `npm test`, then remove `do-not-merge`. |
...
| T67 post-#148 triage refresh | `codex/T67-refresh-after-pr148-triage` / PR pending | review | PR #148 block recorded in TASKS, coordination state, inboxes, and prompt printers | Claude should follow the refreshed #148 blocker in `docs/coordination.json`. |
