...
| T-build-tasks-md-anchors | `claude/T-build-tasks-md-anchors` / PR #133 | blocked | Codex script review passed; local generator check showed generated `TASKS.md` block stale against current `main` | Rebase/update over current `main`, rerun `node scripts/build_tasks_md.mjs --write`, verify idempotence, then rerun script check before removing `do-not-merge`. |
| T-ops-health-summary-route | `claude/T-ops-health-summary-route` / PR #134 | blocked | Focused route test 11/11, syntax check, and whitespace check passed; temp merge showed `backend/index.js` conflict after PR #135 | Rebase/update over current `main`, preserve safe-public/no-user-data posture, rerun focused route test plus `npm test`, then remove `do-not-merge`. |
| T60 export formats picker | `codex/T60-export-formats-picker` | review | Package tests, focused export-format Xcode tests 4/4, full macOS Xcode tests 88/88, generic iOS build, coordination checks, and `git diff --check` passed | No Claude action. The export formats discovery endpoint has an app consumer. |
