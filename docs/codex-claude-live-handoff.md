...
| --- | --- | --- | --- | --- |
| T103 realtime Phase 5b.4 refresh | `codex/T103-phase5b4-merge-refresh` | in-progress | PR #288 merged after Codex patch; `node --test backend/tests/realtime_call_route.test.mjs` 18/18, `node --test scripts/pre_flight.test.mjs` 44/44, `node scripts/pre_flight.mjs`, `node --check backend/index.js`, `git diff --check`, and backend `npm test` 1096 pass / 1 skipped / 0 fail | Phase 5b is complete. Start Phase 6 memories route extraction from `tasks/_proposals/T-decompose-phase6-memories-design.md`; do not open more schema-doc-only PRs unless Codex asks. |
