...
test("[v1-manual-qa] --json emits the five V1 manual gates", () => {
  ...
  assert.match(payload.currentLocalProof, /2026-08-30/);
  assert.match(payload.currentLocalProof, /all five deterministic V1 smokes passed/);
  assert.match(payload.currentLocalProof, /25\/25/);
  assert.match(payload.currentLocalProof, /fail=2 warn=0/);
  assert.match(payload.currentLocalProof, /OPENAI_API_KEY/);
  ...
  assert.ok(releaseFlow.steps.some((step) => /checked-in dedicated iOS entitlement/.test(step)));
  assert.ok(!releaseFlow.steps.some((step) => /Approve a dedicated iOS entitlement/.test(step)));
