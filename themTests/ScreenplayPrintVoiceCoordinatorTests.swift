...
final class ScreenplayPrintVoiceCoordinatorTests: XCTestCase {
    private final class FakePrinter {
        ...
        func pipeline() -> ScreenplayPrintPipeline {
            ScreenplayPrintPipeline(
                ...
                currentTitle: { "Jess's Search" },
                rememberedPrinterName: { [unowned self] in self.rememberedPrinter },
                formatIssue: { [unowned self] _ in self.formatIssue },
                pageCount: { draft in max(1, draft.components(separatedBy: "\n").count / 55 + 1) },
                printToRememberedPrinter: { [unowned self] draft, title in
                    self.rememberedPrints.append((draft, title))
                    return self.rememberedResult
                },
                pickPrinterAndPrint: { [unowned self] draft, title in
    ...
    func testRememberedPrinterAnnouncesThenSpoolsAfterWindow() async {
        ...
        XCTAssertEqual(coordinator.pending, .init(pages: 1, printerName: "Studio Printer", title: "Jess's Search"))
        ...
        XCTAssertEqual(fake.rememberedPrints.first?.title, "Jess's Search")
