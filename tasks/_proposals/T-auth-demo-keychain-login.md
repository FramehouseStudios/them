...
- Remembered credentials are opt-in. The password is stored only as a generic
  password item in Apple Keychain with this-device-only accessibility. It is
  never written to UserDefaults, logs, task state, or backend responses.
...
- Email sign-in shows `Remember email` and `Save password in Apple Keychain`.
- Saving a password implies remembering the email. Turning off remembered
  email clears both Keychain values immediately.
- A `Use Local Demo Account` action shows the fake credentials and attempts a
