...
"""
        ...
        studioProjectStateBoundView
            ...
            .onChange(of: directionOneRightPanelTab) { _, newValue in
                ...
                if newValue == .them, shouldRefreshThemRail {
                    Task {
                        await refreshStudioCreativeInstincts(force: true)
                        await vm.refreshBlockSignal(source: "THEM rail")
                        if !IOThemRuntime.isRunningUITests {
                            await vm.refreshCharacterTraits(source: "THEM rail")
                        }
                        await vm.refreshCraftTwists(source: "THEM rail")
                        await vm.refreshAcceptedCraftTwists(source: "THEM rail")
                    }
                }
                if newValue == .craft {
                    Task {
    ...
    func showBeatsInspectorForStructuralAction() {
        directionOneRightPanelTab = .beats
    }

    func acceptFocusedRewrite(_ request: ScreenplayStudioActionRequest) {
...
private var directionOneScriptEditor: some View {
    ...
    private func studioActionPreviewDiffRowView(_ row: StudioActionPreviewDiffRow) -> some View {
        let tint: Color
        switch row.kind {
        case .unchanged:
    ...
    private func clearCreativePartnerMemory() {
        ...
        Task { @MainActor in
            do {
                ...
                vm.infoText = "Could not clear companion memory. Try again when THEM is online."
            }
        }
    }
...
private var projectsSidebarContent: some View {
    ...
    private func retryFeaturePlannerAction(_ snapshot: ScreenplayFeaturePlannerActionSnapshot) {
        ...
        submitStudioPromptText(
            ...
            successMessage: retrySnapshot.command?.successMessage ?? "Asked THEM to continue the feature plan.",
            clearSeedOnSuccess: false,
            sendingSuggestionID: nil,
            requestIDOverride: requestID,
            completion: { error in
    ...
    func handleSelectionQuickBeatCapture() {
    ...
    func handleSelectedBeatQuickUpdate() {
...
"""
        submitStudioPromptText(
            ...
            source: .typed,
            routingMode: .page,
            successMessage: "Asked THEM to restore the original page write.",
            clearSeedOnSuccess: false,
            sendingSuggestionID: nil
        )
    }
...
"""
        submitStudioPromptText(
            ...
            source: .typed,
            routingMode: .page,
            successMessage: "Asked THEM to rewrite the section from the diff.",
            clearSeedOnSuccess: false,
            sendingSuggestionID: nil
        )
    }
    ...
    private func fullThreadApplyOutlineButton(_ exchange: StudioAskNoteExchange) -> some View {
        ...
        .accessibilityHint("Adds THEM's development beats to Outline and Beats.")
    }

    private var fullThreadApplyOutlineExplanation: some View {
        Text("Adds THEM's development beats to Outline/Beats.")
            .font(.system(size: 10, weight: .regular, design: .default))
    ...
    private func fullStudioThreadExchangeRow(
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            if !exchange.noteTitle.isEmpty || !exchange.noteBody.isEmpty {
                VStack(alignment: .leading, spacing: 5) {
                    Text(ThemProductBrand.presentationLabel(exchange.noteTitle.isEmpty ? ThemProductBrand.name : exchange.noteTitle))
                        .font(.system(size: 12, weight: .semibold, design: .default))
                        .foregroundStyle(Color.herText.opacity(0.72))
    ...
    private func fullThreadExchangeActionButtons(
        ...
        if isRevisedComparison, !isDiffAccepted {
            ...
            .accessibilityHint("Asks THEM to restore the original page write, then returns to the page.")
    ...
    func compactSceneNavigatorLabel(_ heading: String) -> String {
    ...
    private var studioPromptPlaceholder: String {
        ...
        case .voicePin:
            return "Pin a note for THEM to hold"
        }
    }
    ...
    private var studioPromptIntentControl: some View {
        ...
        .accessibilityHint("Changes how THEM responds to the next request.")
...
"""
        ...
        submitStudioPromptText(
            ...
            source: .typed,
            routingMode: .page,
            successMessage: "Asked THEM to make the last write \(preset.title).",
            clearSeedOnSuccess: false,
            sendingSuggestionID: nil
        )
    }
    ...
    func undoLastCommittedWrite(_ committedWrite: ScreenplayCommittedWrite) {
    ...
    func suggestedSceneNavigatorSlugline() -> String {
    ...
    func normalizedSceneInsertSlugline(_ raw: String) -> String {
        var slugline = raw
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .uppercased()
            .replacingOccurrences(of: #"\s+"#, with: " ", options: .regularExpression)
    ...
    private func backfilledStudioExchange(from thread: BackendHistoryThread) -> StudioAskNoteExchange? {
        ...
        let noteTitle = (thread.screenplayNoteTitle ?? "").trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ? (target == .page ? "Wrote to page" : ThemProductBrand.name)
    ...
    private func reloadStudioAskNoteExchange(_ exchange: StudioAskNoteExchange) {
        ...
        vm.infoText = exchange.target == .page
            ? "Loaded this page write back into THEM."
            : "Loaded this Voice Pin ask back into THEM."
    }
    ...
    private func focusStudioRail() {
        ...
        if !studioAskNoteHistory.isEmpty {
            ...
            studioPromptFocused = true
            studioThreadListFocused = false
            vm.infoText = "Focused THEM's prompt."
        }
    }
    ...
    private func submitStudioPromptSeed() {
        ...
        submitStudioPromptText(
            ...
            successMessage: "Prompt sent to THEM.",
...
"""
    ...
    private func applyUITestLaunchActionsIfNeeded() {
        ...
        if let prompt = uiTestLaunchArgumentValue("--ui-auto-submit-voice-source-prompt", in: arguments) {
            ...
            submitStudioPromptText(
                ...
                source: .voice,
                routingMode: .voicePin,
                successMessage: "Voice prompt sent to THEM.",
                clearSeedOnSuccess: true,
                sendingSuggestionID: nil
            )
        }
    ...
    private func applyDebugRouteMetadataSeedIfNeeded() {
        ...
        let companionEntry = StudioAskNoteExchange(
            ...
            target: .voicePin,
            source: .voice,
            noteTitle: ThemProductBrand.name,
