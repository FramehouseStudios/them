...
struct PrintScreenplayIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
            ...
            if let ans = clarification?.trimmingCharacters(in: .whitespacesAndNewlines), !ans.isEmpty {
                ...
                if let url = ScreenplayPrintMemory.rememberedPrinterURL {
                    ...
                    if ok {
                        ...
                        let utter = AVSpeechUtterance(string: "Printing \(pages) pages to \(printerName).")
                ...
                if ok {
                    let utter = AVSpeechUtterance(string: "Printing \(pages) pages.")
                    utter.voice = AVSpeechSynthesisVoice(language: "en-US")
                    AVSpeechSynthesizer().speak(utter)
        ...
        if let url = ScreenplayPrintMemory.rememberedPrinterURL {
            ...
            #if canImport(AVFoundation)
            let guardUtter = AVSpeechUtterance(string: "Printing \(pages) pages to \(printerName) — say cancel to stop, or say print somewhere else to pick another printer.")
            guardUtter.voice = AVSpeechSynthesisVoice(language: "en-US")
            let synth = AVSpeechSynthesizer()
            synth.speak(guardUtter)
            #endif
            // Give user 3s to say “cancel” / “somewhere else” (Siri can re-invoke with pickAlternate), then spool
            try? await Task.sleep(nanoseconds: 3_000_000_000)
            // If app was backgrounded or user cancelled via Siri, the next call will pickAlternate; we just spool remembered now
            let ok = await ScreenplayPrintUI.printSilently(pdfData: pdf, jobName: title, printerURL: url)
            if ok {
                #if canImport(UIKit)
                UINotificationFeedbackGenerator().notificationOccurred(.success)
...
struct ScreenplayShortcuts: AppShortcutsProvider {
    static var appShortcuts: [AppShortcut] {
        ...
        AppShortcut(
            intent: ClarifyStoryIntent(),
            phrases: ["Clarify story in \(.applicationName)", "Answer story question in \(.applicationName)"]
        )
    }
}
