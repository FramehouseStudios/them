    ...
    static var live: ScreenplayPrintPipeline {
        ScreenplayPrintPipeline(
            ...
            printToRememberedPrinter: { draft, title in
                ...
                #elseif os(macOS)
                return ScreenplayPrintServiceMac.print(pdfData: pdf, jobName: title)
                #else
                return false
                #endif
            },
            pickPrinterAndPrint: { draft, title in
                ...
                return ok ? name : nil
                #elseif os(macOS)
                return ScreenplayPrintServiceMac.print(pdfData: pdf, jobName: title) ? "the default printer" : nil
                #else
                return nil
                #endif
            }
...
final class ScreenplayPrintVoiceCoordinator: ObservableObject {
    ...
    init(pipeline: ScreenplayPrintPipeline = .live) {
