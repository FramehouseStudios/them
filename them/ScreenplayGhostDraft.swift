...
/// "interior kitchen day" become `INT. KITCHEN - DAY`; a bare spoken name becomes a
/// centered character cue; everything else stays action.
enum ScreenplayGhostDraft {
    ...
    /// Character cues already on the page, uppercased (dual-dialogue carets stripped via
    /// `characterCueName`), so a spoken name ghosts as a cue even when the recognizer
    /// lower-cases it. Memoized on the draft string because
    ...
    static func characterNames(in draft: String) -> Set<String> {
        ...
        for line in draft.components(separatedBy: .newlines) {
            ...
            let base = ScreenplayEditorElement.characterCueName(trimmed)
